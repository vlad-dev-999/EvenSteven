...
import { useGetEvent, useGetSettlements, useGetBalances } from '@workspace/api-client-react';
  ...
  return (
    <div className="min-h-dvh bg-background transition-page">
      ...
      <main className="max-w-lg mx-auto px-4 py-10 space-y-10">
        {isLoading ? (
          <p className="text-muted-foreground text-sm text-center animate-pulse">Calculating…</p>
        ) : isEven ? (
          /* Everything is even */
          <div className="text-center space-y-4 py-12">
            <p className="font-display text-5xl text-foreground leading-tight">
              Everything is even.
            </p>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              {settlements.map((s, i) => {
                const isMyTransfer = s.fromMemberId === session.memberId;
                const isIncoming = s.toMemberId === session.memberId;

                return (
                  <div
                    key={i}
                    className={`rounded-xl border bg-card px-6 py-5 space-y-3 ${
                      isMyTransfer
                        ? 'border-l-4 border-l-amber-500 border-border'
                        : isIncoming
                        ? 'border-l-4 border-l-green-600 border-border'
                        : 'border-border'
                    }`}
                  >
                    {/* Names + arrow */}
                    <div className="flex items-center gap-3">
                      <div className="text-right flex-1">
                        <p className={`font-medium text-base ${isMyTransfer ? 'text-foreground' : 'text-foreground'}`}>
                          {s.fromMemberName}
                      ...
                      <div className="flex-1">
                        <p className={`font-medium text-base ${isIncoming ? 'text-foreground' : 'text-foreground'}`}>
                          {s.toMemberName}
